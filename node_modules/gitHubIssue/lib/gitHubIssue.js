...
const markdown = require("./marked.min");
 ...
 * 
 */
function GitHubIssues(vscode, context, isV10_7) {
	this._vscode = vscode;
		...
		fs.readFile(pJoin(this._root, ".git", "config"), 'utf8', (error, data) => {
			...
			lines.forEach(line => {
				...
				if (match[1] === 'url') {
					...
					match = match || /git@github.com:([^\/]*)\/([^\/]+?)(?:\.git|$)/.exec(tgt);
					if (match) {
						this._gitHub = {
							owner: match[1],
							repo: match[2]
...
GitHubIssues.prototype.loadVSCodeElements = function() {
	return Promise.resolve()
		...
		.then(() => this._newVersion ? 1 : this.fillRootHTML());
};
...
function markupIssue(issue) {

	let md = "<h1>#" + issue.number + ": <a target='_blank' href='" + issue.web + "'>" + issue.title + "</a></h1>";
	if (issue.labels) {
	...
	if (issue.pull_request) md += "<strong><em>Pull Request</em></strong> &nbsp; ";
	if (issue.user) md += "<strong>User:</strong> " + issue.user.login + "  &nbsp; ";
	if (issue.assignee) md += "<strong>Assignee:</strong> " + issue.assignee.login;
	md += "<br>";
	if (issue.updated && issue.created !== issue.updated) md += "<strong>Updated:</strong> " + issue.updated + "  &nbsp; ";
	md += "<strong>Created:</strong> " + issue.created;
	if (issue.milestone) md += "<br><strong>Milestone: " + issue.milestone.title + "<strong>";
	if (issue.body) {
		md += "<hr>";
...
GitHubIssues.prototype.createHTML = function() {
	return "\n\n<style>" + resources.style + "</style><div class='markdown-body'>" + this._reportedIssues.map(markupIssue)
		.join("\n\n<hr>\n\n") + "</div>";
};

GitHubIssues.prototype.fillRootHTML = function() {
	...
	return ensureDirectory(root)
		...
		.then(() => {
			return new Promise((resolve, reject) => {
				fs.writeFile(pJoin(root, "issueList.md"), this.createHTML(), err => {
					if (err) return reject(err);
					resolve();
				});
			});
